...
def build_recommendations(snapshot: DashboardSnapshot) -> tuple[str, ...]:
    lines: list[str] = []
    weakish = (
        snapshot.strength_weak
        + snapshot.strength_very_weak
        + snapshot.strength_empty
    )
    if weakish:
    ...
    if snapshot.expiring_7:
        lines.append(
            f"{snapshot.expiring_7} entry/entries expire within 7 days."
        )
    if snapshot.cert_expiring_soon:
        lines.append(
            f"{snapshot.cert_expiring_soon} certificate(s) expire soon."
        )
    ...
    if snapshot.empty_usernames:
        lines.append(f"{snapshot.empty_usernames} entr"
                     f"{'y has' if snapshot.empty_usernames == 1 else 'ies have'} "
                     "an empty username.")
    if snapshot.url_http:
        lines.append(
            f"{snapshot.url_http} URL(s) use HTTP instead of HTTPS."
        )
    if snapshot.audit.pwned:
        lines.append(
            f"{snapshot.audit.pwned} password(s) appear in known breaches."
        )
